...
public class Util {
	 ...
	 *            :String 要显示的显示文字
	 * @param type
	 *            :int 显示类型1：仅为确定，2：有“确定�?、�?取消”两个操�?	 * @param handler
	 *            :Handler 传入的需要回调的handler信息，可作为回调方法是用，msg.what = 1时为操作完成状�?�?	 */
	public static void showDialog(Context context, String title, String content, int type, final Handler handler){
		...
		View line_hori_center = view.findViewById(R.id.line_hori_center);// 中竖�?		confirmButton.setVisibility(View.GONE);// 默认隐藏取消按钮
		...
		dialog.setCancelable(true);// 点击返回键关�?	
